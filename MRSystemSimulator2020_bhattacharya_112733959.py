...
class MapReduce:
    ...
    def runSystem(self):
        ...
        runningProcesses = []
        ## <<COMPLETE>>
        chunk = [[] for i in range(self.num_map_tasks)]
        for x in self.data:
            chunk[x[0] % self.num_map_tasks].append(x)
        for data_chunk in chunk:
            p = Process(target=self.mapTask, args=(data_chunk, namenode_m2r, self.use_combiner))
        ...
        ## <<COMPLETE>>
        for x in namenode_m2r:
            to_reduce_task[x[0]].append(x[1])
...
class MeanCharsMR(MapReduce):  # [TODO]
    def map(self, k, v):
        ...
        for char in v:
            if char.isalpha():
                ...
                pairs.append((char, (1, 'MEAN: ' + str(0.0), 'STD_DEV: ' + str(0.0), 0, k, 0)))
        for char in alphabets:
            pairs.append((char, (0, 'MEAN: ' + str(0.0), 'STD_DEV: ' + str(0.0), 0, k, 0)))
        return pairs
    ...
    def reduce(self, k, vs):
        ...
        sumSquare = 0.0
        for key, values in vsPerDoc.items():
            if vs:
                sum += np.sum(values)
                sumSquare += np.square(np.sum(values))
        ...
        std = np.sqrt((count * sumSqr - sum * sum) / (count * (count - 1)))

        return (k, (sum, 'MEAN: ' + str(round(mean, 4)), 'STD_DEV: ' + str(round(std, 4)), sumSqr, 1, count))
